...
export class DataService {
  ...
  categories = {
    decision: {
      color: '#2AB3AB', subCategories: [
        { name: 'Results', apps: [] },
        { name: 'Value Opportunities', apps: [] }
      ]
    },
    planning: {
      color: '#3381FD', subCategories: [
        { name: 'Make it Right', apps: [] },
        { name: 'Make it Magic', apps: [] },
        { name: 'Make it Work', apps: [] },
      ]
    },
    activation: {
      color: '#62D079', subCategories: [
        { name: 'Media', apps: [] },
        { name: 'Content', apps: [] },
        { name: 'Channel', apps: [] },
      ]
    ...
    measurement: {
      color: '#FDC228', subCategories: [
        { name: 'Measurement', apps: [] },
        { name: 'Reporting', apps: [] },
      ]
    },
  }
  ...
  sendData(data, db) {
    console.log(db)
    this.http.post('http://localhost:8080/api/users', data).subscribe(r => {
      console.log(r)
    })
  }

  getApps() {
    this.http.get('http://localhost:8080/api/apps'
      , {
        params: {
      ...
      Object.values(r).forEach(d => {
        if (this.user.installed.includes(d.id)) {
          this.apps.installed.push(d);
        } else {
          this.apps.available.push(d);
        }
        this.apps.all.push(d);
  ...
  addApp(app) {
    let inArray = false;
    this.user.installed.forEach(installed => {
      if (installed === app) { inArray = true }
    });
    if (!inArray) { this.user.installed.push(app) }
    this.http.put('http://localhost:8080/api/app/add', { app: this.user.installed, user: this.user.user }).subscribe(d => {
      this.apps.available.forEach((available, i) => {
        if (available.id === app) {
          this.apps.installed.push(this.apps.available[i])
          this.apps.available.splice(i, 1);
        }
      })
    });
  }
